...
#write adjectives,adverbs and verbs in pos_tag1.csv using textblob                  
df_len = df.shape[0]
file = open("pos_tag1.csv","w")
...
for row in range(0,df_len):
   ...
   for i in range(0,tags_len):
       if str(tags[i][1]).find('JJ') == 0 or str(tags[i][1]).find('RB') == 0 or str(tags[i][1]).find('VB') == 0:
           score = TextBlob(str(tags[i][0])).sentiment.polarity
           score = math.ceil((((score + 1) * 10) / 2) - 5)
           if score != 0:
               line += str(score) + " " + str(tags[i][0])+" "
   line+="\n"
   print(line)
   file.write(line)
file.close()
...
ratio_min.to_csv("unigram_ratio_min.csv")
...
xx = vectorizer.transform([df_unigrams['tweet'].tolist()[0]])
...
pattern_ratio_min.to_csv("pattern_ratio_min.csv")
...
with open('my_svclassifier.pkl', 'wb') as fid:
